...
void MapCtl::MapDraw(VECTOR2 camPos)
{
	...
	DrawGraph(780, 8, IMAGE_ID("Image/number.png")[(Score / 100)], true);
	DrawGraph(815, 8, IMAGE_ID("Image/number.png")[(Score/10 )%10], true);
	DrawGraph(850, 8, IMAGE_ID("Image/number.png")[Score % 10], true);
...
void MapCtl::MapNearDraw(VECTOR2 camPos)
{
	...
	for (int y = 0; y < ChipCnt.y; y++)
	{
		...
		for (leftX; leftX < rightX; leftX++)
		{
			if (mapID[y][leftX] == CHIP_GRASS1 || mapID[y][leftX] == CHIP_GRASS2 || mapID[y][leftX] == CHIP_GRASS3)
			{
				DrawGraph(leftX * 32 + GameDrawOffset().x, y * 32 + GameDrawOffset().y, IMAGE_ID("Image/mt.png")[mapID[y][leftX]], true);		//’Ç‰Á
			}
		}
		leftX = (int)((camPos.x / 32) - (VIEW_CHIP_CNT_X / 2) - 1);
	}
...
bool MapCtl::MapLoad(void)
{
	...
	else if (mapType == 2)fileName = "data/‚È‚ñ‚©.fmf";			//’Ç‰Á
...
void MapCtl::SetMapType(int no)
{
	...
	else if (mapType == 1 || mapType == 2)				//’Ç‰Á
	{
		ChipCnt = {700,24};
	}
...
MapCtl::MapCtl()
{
	...
	ImageMng::GetInstance().GetID("Image/mt2.jpg", { 32,32 }, { 27,1 }, { 0,0 });		//’Ç‰Á
