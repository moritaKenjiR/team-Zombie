...
bool EnemyAI::Dijkstra(const VECTOR2& start, const VECTOR2& goal)
{
	...
	if (!((Range((int)lpMapCtl.GetChipType(start), CHIP_TYPE::CHIP_FIRE, CHIP_TYPE::CHIP_FLAG_OBJ3) || lpMapCtl.GetChipType(start) == CHIP_BLANK)
		&& ((Range((int)lpMapCtl.GetChipType(goal), CHIP_TYPE::CHIP_FIRE, CHIP_TYPE::CHIP_FLAG_OBJ3) || lpMapCtl.GetChipType(goal) == CHIP_BLANK)))) return false;
